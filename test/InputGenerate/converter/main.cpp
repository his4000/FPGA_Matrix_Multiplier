...
int main(int argc, char **argv)
{
	...
	ofstream log;
	log.open("generate.log");

	//print
	cout 	<< "(baseline) (CPU code) (your code)\n";
	log 	<< "(baseline) (CPU code) (your code)\n";
	for(size_t i = 0 ; i < matrix_size ; i++)
	{
		...
		if(your_error > ERROR_THRESHOLD || your_error < -ERROR_THRESHOLD)
		{
			cerr	<< "Your result exceeded error threshold("
				<< ERROR_THRESHOLD * 100 << "%)\n";
			log		<< "Your result exceeded error threshold("
				<< ERROR_THRESHOLD * 100 << "%)\n";
		}
	}
	...
	log	 << "Elapsed time(Your IP) : " << t_you << endl;
	log	 << "Elapsed time(ARM CPU) : " << t_arm << endl;
	log.close();
